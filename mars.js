const scene = new THREE.Scene(); // Création de la scène (espace pour placer des objets, la lumière et la caméra)
...
const material = new THREE.MeshPhongMaterial(); // Définit l'aspect, la texture
...
material.map = new THREE.TextureLoader().load('res/diffuse.jpg'); // Superposition des images de texture
material.bumpMap = new THREE.TextureLoader().load('res/bump.jpg');     
material.bumpScale = 0.015; // Indice de l'atténuation de relief

const starsGeometry = new THREE.SphereGeometry(4, 32, 32); // Ajout et paramétrage du ciel étoilé
...
const animate = () => { // Une boucle de rendu de scène
